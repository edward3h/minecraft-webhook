...
public class Tailer {
    ...
    [INFO] Player disconnected: Foxer191, xuid: 2535428717109723
        */
    private static final Pattern playerEvent = Pattern.compile(" Player ([^ ]*connected): (.*), xuid: (\\d+)");
    ...
    private class InitialCallback extends ResultCallback.Adapter<Frame> {
        ...
        public void onComplete() {
            if (foundWorld) {
                reaper.check(
                        LOGGER,
                        eventPublisher.publishEventAsync(new MinecraftServerEvent(
    ...
    private class FollowCallback extends ResultCallback.Adapter<Frame> {
        ...
        public void onNext(Frame frame) {
            ...
            if (matcher.find()) {
                ...
                reaper.check(
                        LOGGER,
                        eventPublisher.publishEventAsync(new MinecraftPlayerEvent(
        ...
        public void onComplete() {
            reaper.check(
                    ...
                    eventPublisher.publishEventAsync(
                            new MinecraftServerEvent(EventType.SERVER_STOPPED, containerId, containerName, worldName)));
